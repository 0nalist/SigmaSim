extends Node  # Change from SceneTree to Node

const NPC: GDScript = preload("res://components/npc/npc.gd")
const ExFactorLogic: GDScript = preload("res://components/popups/ex_factor_logic.gd")

func _ready() -> void:
	var npc_mgr: Object = Engine.get_singleton("NPCManager")
	var db_mgr: Object = Engine.get_singleton("DBManager")

	var npc1_idx: int = 10101
	var npc2_idx: int = 20202

	# Stub out database writes
	db_mgr.save_npc = func(_i, _n): pass

	var npc1: Object = NPC.new()
	npc1.relationship_stage = NPCManager.RelationshipStage.TALKING
	npc1.exclusivity_core = NPCManager.ExclusivityCore.MONOG

	var npc2: Object = NPC.new()
	...
	var logic: Node = ExFactorLogic.new()
	add_child(logic)  # works now, since self is a Node
	logic.setup(npc1, npc1_idx)
	logic.request_next_stage_primary()
	await get_tree().process_frame  # works now too
