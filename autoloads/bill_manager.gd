		...
		if popup and popup.visible:
			if popup.bill_name == "Credit Card":
				if PortfolioManager.pay_down_credit(popup.amount):
					_set_credit_card_balance(
						PortfolioManager.credit_used, PortfolioManager.credit_limit
					)
...
func add_debt_resource(resource: Dictionary) -> void:
	...
	match name:
		...
		"Student Loan":
			interval = (
				TimeManager.get_days_in_month(TimeManager.current_month, TimeManager.current_year)
				* 1440
			)
		"Payday Loan":
			interval = 1 * 1440
	...
	match name:
		...
		"Student Loan":
			if PortfolioManager.pay_with_cash(amount):
				PortfolioManager.set_student_loans(
					max(PortfolioManager.get_student_loans() - amount, 0.0)
				)
...
func _schedule_payday_loan_bill(amount: float) -> void:
	...
	if autopay_enabled and attempt_to_autopay("Payday Loan"):
		...
		pending_bill_data[date_key].append({"bill_name": "Payday Loan", "amount": amount})
		show_due_popups()
	...
	for entry in temp:
		if typeof(entry) == TYPE_DICTIONARY:
			...
			if res.has("compounds_in") or res.has("compound_interval"):
				...
				var minutes := int(
					res.get("minutes_until_due", res.get("days_until_due", 0) * 1440)
				)
...
var lifestyle_options := {
	...
	[
		...
		{"name": "Occasional Netflix Night", "cost": 35, "effects_label": "+0 Mood", "effects": {}},
		{
	...
	[
		...
		{"name": "Public Transit", "cost": 35, "effects_label": "+0 Mood", "effects": {}},  # $5/day
		{
