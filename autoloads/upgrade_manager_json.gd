extends Node
## UpgradeManager loads upgrade definitions from JSON files and
## exposes helpers for querying, costing, and purchasing upgrades.
##
## Upgrades support multi-currency costs. Each level's cost is a
## dictionary mapping currency names to amounts. Costs can be supplied
## as an array (one dictionary per level) or as a single dictionary for
## infinite upgrades. If `scale_by_formula` is true, `cost_formula`
## provides either a single expression returning a Dictionary or a
## Dictionary of expressions, one per currency. Expressions receive the
## variables `level` (1-based), `base_cost` (Dictionary), and
## `prev_cost` (Dictionary of the previous level).
...
func _validate_upgrade(data: Dictionary, file_path: String) -> bool:
	...
	var currencies := _get_known_currencies()
	var cost_dicts: Array = []
	if typeof(data["cost_per_level"]) == TYPE_ARRAY:
		cost_dicts = data["cost_per_level"]
	else:
		cost_dicts = [data["cost_per_level"]]
	for cost_dict in cost_dicts:
		for currency in cost_dict.keys():
			if currency not in currencies:
				push_warning("UpgradeManager: unknown currency '%s' in %s" % [currency, id])
...
func _get_known_currencies() -> Array:
	var list: Array = ["cash"]
	for symbol in PortfolioManager.crypto_owned.keys():
		list.append(symbol)
	return list
