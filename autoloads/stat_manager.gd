...
func _ready() -> void:
	_load_base_stats()
	print("StatManager: connecting to UpgradeManager signals")
	...
	#print("base stats: " + str(base_stats))
...
func _on_upgrades_changed(_id: String = "", _level: int = 0) -> void:
	print("StatManager: _on_upgrades_changed", _id, _level)
	_recalculate_all()

func _recalculate_all() -> void:
	print("StatManager: _recalculate_all start")
	...
	for id in UpgradeManager.player_levels.keys():
		...
		if upg == null:
			continue
		print("StatManager: applying upgrade", id, "level", level)
		for effect in upg.get("effects", []):
			_apply_effect(effect, level)
	for key in stats.keys():
		if old_stats.get(key) != stats[key]:
			print("StatManager: stat", key, "changed to", stats[key])
...
func _apply_effect(effect: Dictionary, level: int) -> void:
	...
	print(
		"StatManager: applying effect to",
		target,
		"op",
		op,
		"value",
