...
func _update_sidebar() -> void:
	...
	if sidebar.has_node("IdLabel"):
		var label: Label = sidebar.get_node("IdLabel")
		label.text = selected_node_id
	if sidebar.has_node("TextEdit"):
		var te: TextEdit = sidebar.get_node("TextEdit")
		te.text = String(data.get("text", ""))
