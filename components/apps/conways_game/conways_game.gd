...
func _cells_has(arr: PackedVector2Array, cell: Vector2i) -> bool:
        return arr.find(cell) != -1

func _cells_insert(arr: PackedVector2Array, cell: Vector2i) -> void:
        if arr.find(cell) == -1:
                arr.append(cell)

func _cells_erase(arr: PackedVector2Array, cell: Vector2i) -> void:
        var idx: int = arr.find(cell)
        if idx != -1:
                arr.remove_at(idx)
...
func _advance_generation() -> void:
        ...
        for cell in neighbor_counts.keys():
                ...
                var alive: bool = _cells_has(cells, cell)
                if alive:
                        if count == 2 or count == 3:
                                _cells_insert(new_cells, cell)
                else:
                        if count == 3:
                                _cells_insert(new_cells, cell)
        cells = new_cells
...
func _toggle_cell(cell: Vector2i) -> void:
        var alive: bool = _cells_has(cells, cell)
        if alive:
                _cells_erase(cells, cell)
        else:
                _cells_insert(cells, cell)
        queue_redraw()
